...
// CAUTION: this function only works for arguments in the range [-0.25f; 0.25f]!
static void my_sincosm1pi (float a, float *restrict res)
  {
  float s = a * a;
  /* Approximate cos(pi*x)-1 for x in [-0.25f,0.25f] */
  float r =     -1.0369917389758117e-4f;
  r = fmaf (r, s,  1.9294935641298806e-3f);
  r = fmaf (r, s, -2.5806887942825395e-2f);
  r = fmaf (r, s,  2.3533063028328211e-1f);
  r = fmaf (r, s, -1.3352627688538006e+0f);
  r = fmaf (r, s,  4.0587121264167623e+0f);
  r = fmaf (r, s, -4.9348022005446790e+0f);
  float c = r*s;
  /* Approximate sin(pi*x) for x in [-0.25f,0.25f] */
  r =             4.6151442520157035e-4f;
  r = fmaf (r, s, -7.3700183130883555e-3f);
  r = fmaf (r, s,  8.2145868949323936e-2f);
  r = fmaf (r, s, -5.9926452893214921e-1f);
  r = fmaf (r, s,  2.5501640398732688e+0f);
  r = fmaf (r, s, -5.1677127800499516e+0f);
  s = s * a;
  r = r * s;
  s = fmaf (a, 3.1415926535897931e+0f, r);
  res[0] = c;
  res[1] = s;
  }
...
NOINLINE static void calc_first_octant(size_t den, float * restrict res)
  ...
  res[0]=1.f; res[1]=0.f;
  if (n==1) return;
  size_t l1=(size_t)sqrtf(n);
  for (size_t i=1; i<l1; ++i)
    my_sincosm1pi((2.f*i)/den,&res[2*i]);
  size_t start=l1;
  while(start<n)
    {
    float cs[2];
    my_sincosm1pi((2.f*start)/den,cs);
    res[2*start] = cs[0]+1.f;
    res[2*start+1] = cs[1];
    size_t end = l1;
    ...
    for (size_t i=1; i<end; ++i)
      ...
      res[2*(start+i)] = ((cs[0]*csx[0] - cs[1]*csx[1] + cs[0]) + csx[0]) + 1.f;
...
NOINLINE static size_t largest_prime_factor (size_t n)
  ...
  size_t limit=(size_t)sqrtf(n+0.01f);
  for (size_t x=3; x<=limit; x+=2)
  while (((tmp=(n/x))*x)==n)
    {
    res=x;
    n=tmp;
    limit=(size_t)sqrtf(n+0.01f);
    }
  if (n>1) res=n;
...
NOINLINE static float cost_guess (size_t n)
  {
  const float lfp=1.1f; // penalty for non-hardcoded larger factors
  size_t ni=n;
  float result=0.f;
  size_t tmp;
  while (((tmp=(n>>1))<<1)==n)
...
NOINLINE static void pass7(size_t ido, size_t l1, const cmplx * restrict cc,
  ...
  {
  const size_t cdim=7;
  const float tw1r= 0.623489801858733530525f,
...
NOINLINE static void pass11 (size_t ido, size_t l1, const cmplx * restrict cc,
  ...
  const float tw1r =        0.8412535328311811688618f,
...
NOINLINE static void radf3(size_t ido, size_t l1, const float * restrict cc,
  ...
  {
  const size_t cdim=3;
  static const float taur=-0.5f, taui=0.86602540378443864676f;
...
NOINLINE static void radf5(size_t ido, size_t l1, const float * restrict cc,
  ...
  {
  const size_t cdim=5;
  static const float tr11= 0.3090169943749474241f, ti11=0.95105651629515357212f,
...
NOINLINE static void radb2(size_t ido, size_t l1, const float * restrict cc,
  ...
  if ((ido&1)==0)
    for (size_t k=0; k<l1; k++)
      {
      CH(ido-1,k,0) = 2.f*CC(ido-1,0,k);
      CH(ido-1,k,1) =-2.f*CC(0    ,1,k);
      }
  if (ido<=2) return;
  for (size_t k=0; k<l1;++k)
    for (size_t i=2; i<ido; i+=2)
...
NOINLINE static void radb3(size_t ido, size_t l1, const float * restrict cc,
  ...
  for (size_t k=0; k<l1; k++)
    {
    float tr2=2.f*CC(ido-1,1,k);
    float cr2=CC(0,0,k)+taur*tr2;
    CH(0,k,0)=CC(0,0,k)+tr2;
    float ci3=2.f*taui*CC(0,2,k);
...
NOINLINE static void radb4(size_t ido, size_t l1, const float * restrict cc,
  ...
  for (size_t k=0; k<l1; k++)
    ...
    float tr3=2.f*CC(ido-1,1,k);
    float tr4=2.f*CC(0,2,k);
...
NOINLINE static void radbg(size_t ido, size_t ip, size_t l1,
  ...
  for (size_t j=1, jc=ip-1; j<ipph; ++j, --jc)   // 108
    ...
    for (size_t k=0; k<l1; ++k)
      {
      CH(0,k,j ) = 2.f*CC(ido-1,j2,k);
      CH(0,k,jc) = 2.f*CC(0,j2+1,k);
      }
    }
...
WARN_UNUSED_RESULT NOINLINE static int rfftp_comp_twiddle (rfftp_plan plan)
  ...
  for (size_t k=0; k<plan->nfct; ++k)
    ...
    if (ip>5) // special factors required by *g functions
      ...
      plan->fct[k].tws[0] = 1.f;
      plan->fct[k].tws[1] = 0.f;
...
/* initialize b_k */
  ...
  if (cfftp_forward(plan->plan,plan->bkf,1.f)!=0)
    { DEALLOC(tmp); DEALLOC(plan->mem); DEALLOC(plan); return NULL; }
  DEALLOC(tmp);
...
/* inverse FFT */
  if (cfftp_backward (plan->plan,akf,1.f)!=0)
    { DEALLOC(akf); return -1; }
...
typedef struct cfft_plan_f32_i
  {
  cfftp_plan packplan;
...
cfft_plan_f32 make_cfft_plan_f32 (size_t length)
  {
  if (length==0) return NULL;
  cfft_plan_f32 plan = RALLOC(cfft_plan_f32_i,1);
  if (!plan) return NULL;
  plan->blueplan=0;
  plan->packplan=0;
  if ((length<50) || (largest_prime_factor(length)<=sqrtf(length)))
    {
  ...
  float comp2 = 2.f*cost_guess(good_size(2*length-1));
  comp2*=1.5f; /* fudge factor that appears to give good overall performance */
...
typedef struct rfft_plan_f32_i
  {
  rfftp_plan packplan;
  fftblue_plan blueplan;
  } rfft_plan_f32_i;
...
rfft_plan_f32 make_rfft_plan_f32 (size_t length)
  {
  if (length==0) return NULL;
  rfft_plan_f32 plan = RALLOC(rfft_plan_f32_i,1);
  if (!plan) return NULL;
  plan->blueplan=0;
  plan->packplan=0;
  if ((length<50) || (largest_prime_factor(length)<=sqrtf(length)))
  ...
  float comp1 = 0.5f*cost_guess(length);
  float comp2 = 2.f*cost_guess(good_size(2*length-1));
  comp2*=1.5f; /* fudge factor that appears to give good overall performance */
  if (comp2<comp1) // use Bluestein
    {
